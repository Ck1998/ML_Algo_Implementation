...
class GammaRegressorTest:
    ...
    def __init__(self, dataset_id: int, prefix=False):
        ...
        self.clf = GammaRegressor(alpha=0.1)

    def save(self, suffix):
        save_model(model=self.clf,
                   file_name=f"lasso_regression_predictor_{DATASET_MAPPINGS[self.dataset_id]['name']}_{suffix}.model",
                   prefix=self.prefix)
...
if __name__ == "__main__":
    GammaRegressorTest(dataset_id=6, prefix=True).run()
